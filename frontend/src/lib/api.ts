...
export const apiEndpoints = {
  ...
  startWorkspaceHandoff: (targetUserId: string, nextPath?: string) =>
    api.post("/api/auth/workspace-handoff/start", { targetUserId, nextPath }),
  ...
  r3dDeleteProduct: (id: string) => api.delete(`/api/rotation3d/products/${id}`),
  ...
  r3dSetFeatured: (
    id: string,
    changes: { featured?: boolean; heroFeatured?: boolean; featuredRank?: number },
  ) => api.patch(`/api/rotation3d/products/${id}/feature`, changes),
  ...
  driftCreatorSignup: (name?: string, confirm?: boolean, accountType?: string, ownPage?: boolean) =>
    api.post("/api/drift/creator/signup", { name, confirm: confirm === true, accountType, ownPage: ownPage === true }),
  ...
  driftCreatePageInvite: (email: string, role?: string) => api.post("/api/drift/my/page/invites", { email, role }),
  driftRevokePageInvite: (id: string) => api.delete(`/api/drift/my/page/invites/${id}`),
  driftPagePeople: () => api.get("/api/drift/my/page/people"),
  driftUpdatePageMember: (id: string, role: string) => api.patch(`/api/drift/my/page/members/${encodeURIComponent(id)}`, { role }),
  driftRemovePageMember: (id: string) => api.delete(`/api/drift/my/page/members/${encodeURIComponent(id)}`),
  driftCreatorPages: () => api.get("/api/drift/creator/pages"),
  ...
  driftTourReel: (id: string, layout: "full" | "landscape" | "framed") => api.get(`/api/drift/my/flows/${id}/reel`, { params: { layout } }),
  driftStartTourReel: (id: string, layout: "full" | "landscape" | "framed") =>
    api.post(`/api/drift/my/flows/${id}/reel`, null, { params: { layout } }),
  driftTourReelFile: (id: string, layout: "full" | "landscape" | "framed") =>
  ...
  driftPublicLandingHero: (brandSlug?: string | null) =>
    api.get("/api/drift/public/landing-hero", brandSlug ? { params: { brand: brandSlug } } : undefined),
  ...
  superadminSetOrgPlan: (id: string, plan: "DEMO" | "PAID", trialDays?: number) =>
    api.put(`/api/superadmin/organizations/${id}/plan`, { plan, trialDays }),
